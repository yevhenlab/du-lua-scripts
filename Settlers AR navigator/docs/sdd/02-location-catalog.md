...
- `id`: optional numeric DU identifier for the represented system, celestial body, construct, or other place. Planet and moon nodes use their DU body ID; construct nodes use their construct ID. A group normally has no ID.
- `type`: required single node classification used for behavior and default icon selection. Structural values include `known-space`, `space`, `system`, `planet`, `moon`, `asteroid`, `location-group`, `construct`, and `zone`; semantic place values include `market`, `base`, `mining`, `outpost`, `settlement`, `mission`, `quest`, `showroom`, `event`, `parking`, and `station`.
...
- `radius`: surface radius in metres for a planet or moon.
